...
public class RelatorioVendaServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        ResultSet valorTransacaoRS = SQL.query("SELECT valorTransacao FROM Transacao WHERE id = " + idTransacao);
        Double valorTransacao;
        
        while(valorTransacaoRS.next()){
            valorTransacao = valorTransacaoRS.getDouble("valorTransacao");
            ResultSet dataTransacaoRS = SQL.query("SELECT dataTransacao FROM Transacao WHERE id = " + idTransacao);
            Calendar dateTransacao;
            
            while(dataTransacaoRS.next()){
                dateTransacao = Data.dateToCalendar(dataTransacaoRS.getDate("dataTransacao"));
                ResultSet idItemTransacaoRS = SQL.query("SELECT idItemTransacao FROM Transacao WHERE id = " + idTransacao);
                Integer idItemTransacao;
                
                while(idItemTransacaoRS.next()){
                    idItemTransacao = idItemTransacaoRS.getInt("idItemTransacao");
                    ResultSet nomeProdutoRS = SQL.query("SELECT nomeProduto FROM Produto WHERE idProduto = " + idItemTransacao);
                    String nomeProduto;
                    
                    while(nomeProdutoRS.next()){
                        nomeProduto = nomeProdutoRS.getString("nomeProduto");
                        
                        RelatorioVendas relatorio = new RelatorioVendas(valorTransacao,
                                nomeProduto, dateTransacao.get(Calendar.DATE),
                                dateTransacao.get(Calendar.MONTH), dateTransacao.get(Calendar.YEAR));
